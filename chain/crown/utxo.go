...
func (tx *Tx) Serialize() (pack.Bytes, error) {	
	...
	("=========================")
	("En el serialize de utxo.go crown ", pack.NewBytes(buf.Bytes()))
...
func (txBuilder TxBuilder) BuildTx(inputs []utxo.Input, recipients []utxo.Recipient) (utxo.Tx, error){
	...
	("=============================================")
	("En el BuildTx crown.go inputs", inputs)
	("En el BuildTx crown.go recipients", recipients)
	("En el BuildTx crown.go intial msgTx", msgTx)
	// Inputs
	for _, input := range inputs {
		hash := chainhash.Hash{}
		("En el BuildTx crown.go each input hash", input.Hash.String())
	...
	for _, recipient := range recipients {
		...
		("En el BuildTx crown.go current addr", addr)
		if err != nil {
			return nil, err
		}
		script, err := txscript.PayToAddrScript(addr.BitcoinAddress())
		("En el BuildTx crown.go current script", string(script))
		if err != nil {
			return nil, err
		}
		...
		("En el BuildTx crown.go current value", value)
		msgTx.AddTxOut(wire.NewTxOut(value, script))
		("En el BuildTx crown.go each change of msgTx", msgTx)
	}
	("=============================================")
